...
# Daily MongoDB Backup script schedule (do not use "/" in the end of directories)
mongodb_backup: true
...
mongodb_backup_store_dir: "{{ mongodb_storage_dbpath }}/backup/storage"
mongodb_backup_store_days: 0   # f the value is 0, the script will not copy the backups to the storage directory, and will keep only one local copy.
